...
const createJob = async (req, res) => {
  try {
    const mgr = await db.query(
      'SELECT cm.id, cm.company_id FROM company_managers cm WHERE cm.user_id = $1',
      [req.user.id]
    )
    if (!mgr.rows.length) return res.status(404).json({ success: false, message: 'Not found' })

    const { company_id, id: manager_id } = mgr.rows[0]
  ...
  try {
    ...
    const result = await db.query(`
      UPDATE job_postings SET
        ...
        skills_required      = COALESCE($11, skills_required),
...
const getJobApplicants = async (req, res) => {
  try {
    ...
    const owns = await db.query(
      'SELECT id FROM job_postings WHERE id = $1 AND company_id = $2',
      [req.params.jobId, mgr.rows[0].company_id]
    )
    if (!owns.rows.length) return res.status(403).json({ success: false, message: 'Access denied' })
  ...
  try {
    ...
    if (empInfo.rows.length) {
      await db.query(
        'INSERT INTO notifications (user_id, title, message, type, reference_id) VALUES ($1, $2, $3, $4, $5)',
        [
          empInfo.rows[0].user_id,
          'Application Update',
          `Your application for "${empInfo.rows[0].title}" has been ${status}`,
          'application',
          req.params.appId,
        ]
      )

...
//   try {
//     const [revenue, monthly] = await Promise.all([
//       db.query("SELECT COALESCE(SUM(amount), 0) as total_revenue, COUNT(*) as total_payments FROM payments WHERE status = 'completed'"),
//       db.query(`
//         SELECT TO_CHAR(created_at, 'Mon YYYY') as month, COALESCE(SUM(amount), 0) as revenue
//         FROM payments WHERE status = 'completed' AND created_at >= NOW() - INTERVAL '6 months'
...
// ─── REVENUE ─────────────────────────────────────────────────────────────────
// Fixed: frontend expects data.summary.total_revenue, data.summary.total_payments
// and data.monthly (not data.monthly_revenue)
const getRevenue = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        summary:  summary.rows[0],   // { total_revenue, total_payments, monthly_revenue }
        monthly:  monthly.rows,      // [{ month, revenue }, ...]
        by_plan:  byPlan.rows        // [{ plan, revenue, count }, ...]
      }
    })
  } catch (err) {
    console.error(err)

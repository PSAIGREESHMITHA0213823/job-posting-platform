...
// //   try {
// //     const {
// //       full_name, phone, headline, summary, location,
// //       date_of_birth, gender, skills, experience_years,
// //       education, work_experience, social_links,
// //       linkedin_url, github_url, portfolio_url, certifications
// //     } = req.body

// //     const files = req.files || {}
// //     const resume_url  = files.resume  ? `/uploads/resumes/${files.resume[0].filename}`  : undefined
// //     const avatar_url  = files.avatar  ? `/uploads/avatars/${files.avatar[0].filename}`  : undefined

// //     const sets = []
// //     const vals = []
// //     let i = 1

// //     const fields = { full_name, phone, headline, summary, location, date_of_birth, gender, experience_years, linkedin_url, github_url, portfolio_url }
// //     for (const [key, val] of Object.entries(fields)) {
// //       if (val !== undefined) { sets.push(`${key} = $${i++}`); vals.push(val) }
// //     }
...
// //     if (skills) {
// //       sets.push(`skills = $${i++}`)
// //       vals.push(Array.isArray(skills) ? skills : skills.split(',').map(s => s.trim()).filter(Boolean))
// //     }
// //     if (certifications) {
// //       sets.push(`certifications = $${i++}`)
// //       let parsed
// //       try { parsed = JSON.parse(certifications) } catch { parsed = certifications.split(',').map(s => s.trim()).filter(Boolean) }
// //       vals.push(parsed)
// //     }
// //     if (education)       { sets.push(`education = $${i++}`);       vals.push(JSON.stringify(education)) }
// //     if (work_experience) { sets.push(`work_experience = $${i++}`); vals.push(JSON.stringify(work_experience)) }
...
// //         jp.salary_min, jp.salary_max, jp.salary_currency, jp.experience_required,
// //         jp.skills_required, jp.category, jp.openings, jp.application_deadline,
// //         jp.views, jp.created_at, c.name as company_name, c.logo_url, c.city,
...
// //     await db.query(
// //       'UPDATE job_postings SET views = views + 1 WHERE id = ANY($1::uuid[])',
// //       [result.rows.map(r => r.id)]
// //     )

// //     const count = await db.query(
// //       `SELECT COUNT(DISTINCT jp.id) FROM job_postings jp JOIN companies c ON jp.company_id = c.id ${where}`,
// //       params.slice(0, -2)
// //     )
...
// //       'SELECT id FROM job_applications WHERE job_id = $1 AND employee_id = $2', [job_id, empId]
// //     )
// //     if (alreadyApplied.rows.length) return res.status(409).json({ success: false, message: 'Already applied to this job' })
...
// //         job_id, employee_id, cover_letter, resume_url,
// //         applicant_name, applicant_email, applicant_phone, applicant_location,
// //         linkedin_url, github_url, portfolio_url,
// //         availability, notice_period, expected_salary,
// //         education, work_experience, projects, certifications, skills
...
// //         availability || null, notice_period || null, expected_salary || null,
// //         parseJson(education), parseJson(work_experience), parseJson(projects),
// //         parseCerts(certifications), parseSkills(skills)
// //       ]
// //     )

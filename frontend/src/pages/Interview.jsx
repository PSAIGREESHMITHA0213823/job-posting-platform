...
//             { icon: '💻', text: 'Technical questions — explain concepts and what you have built' },
//             { icon: '🧠', text: 'HR questions — time management, pressure, teamwork' },
...
const VIOLATION_INFO = {
tab_switch: { icon: <i className="bi bi-eye-slash-fill" />, title: 'Tab Switch Detected', body: 'You switched to another tab. This is strictly not allowed.' },
window_blur: { icon: <i className="bi bi-window-sidebar" />, title: 'Window Focus Lost', body: 'You navigated away from the interview window.' },
paste_attempt: { icon: <i className="bi bi-clipboard-x-fill" />, title: 'Paste Blocked', body: 'Copy-pasting is not allowed. Please type your answer manually.' },
fullscreen_exit: { icon: <i className="bi bi-fullscreen-exit" />, title: 'Fullscreen Required', body: 'You exited fullscreen. Please return to fullscreen to continue.' },
devtools_open: { icon: <i className="bi bi-bug-fill" />, title: 'DevTools Detected', body: 'Browser developer tools are not allowed during the interview.' },
};

function ViolationModal({ violation, count, onClose, onFullscreen }) {
if (!violation) return null;
const info = VIOLATION_INFO[violation] || { icon: <i className="bi bi-exclamation-triangle-fill" />, title: 'Violation', body: 'An integrity violation was detected.' };
const remaining = MAX_WARNINGS - count;
return (
<div style={{
position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.82)',
...
{remaining > 0
? <><i className="bi bi-exclamation-triangle-fill me-1" /> {remaining} warning{remaining > 1 ? 's' : ''} left before your interview is auto-submitted.</>
: <><i className="bi bi-stop-circle-fill me-1" /> This was your final warning. Your interview is being submitted now.</>}
</div>
{violation === 'fullscreen_exit' ? (
<button onClick={onFullscreen} style={btnStyle('#2563eb')}><i className="bi bi-arrows-fullscreen me-2" />Return to Fullscreen</button>
) : (
<button onClick={onClose} style={btnStyle('#374151')}><i className="bi bi-check-lg me-2" />I Understand — Continue</button>
)}
</div>
</div>
);
...
<div style={{ fontSize: 60, marginBottom: 16 }}><i className="bi bi-x-octagon-fill" style={{ color: '#ef4444' }} /></div>
<h3 style={{ color: '#ef4444', fontWeight: 700, marginBottom: 10 }}>Interview Terminated</h3>
<p style={{ color: '#fca5a5', fontSize: 15, lineHeight: 1.6, marginBottom: 24 }}>
Your interview has been automatically submitted due to {MAX_WARNINGS} integrity violations.
</p>
...
<p style={{ color: '#ef4444', fontWeight: 600, fontSize: 13, marginBottom: 10 }}>Violation Log</p>
{violations.map((v, i) => {
const info = VIOLATION_INFO[v.type] || { icon: <i className="bi bi-exclamation-triangle-fill" />, title: v.type };
return (
<div key={i} style={{ display: 'flex', gap: 10, marginBottom: 6, fontSize: 13, color: '#fca5a5' }}>
<span style={{ color: '#6b7280', minWidth: 64 }}>{v.time}</span>
<span>{info.icon} {info.title}</span>
...
<div style={{ fontSize: 52, marginBottom: 16 }}><i className="bi bi-display" /></div>
<h3 style={{ color: '#f9fafb', fontWeight: 700, marginBottom: 10 }}>Fullscreen Mode Required</h3>
<p style={{ color: '#9ca3af', fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
This interview is monitored for integrity. The following are strictly prohibited:
</p>
<div style={{ textAlign: 'left', marginBottom: 28 }}>
{[
<><i className="bi bi-x-circle-fill me-2" /> Switching tabs or windows</>,
<><i className="bi bi-x-circle-fill me-2" /> Copy / paste of any kind</>,
<><i className="bi bi-x-circle-fill me-2" /> Right-clicking anywhere</>,
<><i className="bi bi-x-circle-fill me-2" /> Opening browser DevTools</>,
<><i className="bi bi-x-circle-fill me-2" /> Exiting fullscreen mode</>,
<><i className="bi bi-exclamation-triangle-fill me-2" /> {MAX_WARNINGS} violations = automatic submission</>,
].map((r, i) => (
<div key={i} style={{
background: '#1f2937', borderRadius: 8, padding: '10px 14px',
marginBottom: 8, color: '#d1d5db', fontSize: 14
...
<button onClick={onEnter} style={btnStyle('#7c3aed')}>
<i className="bi bi-arrows-fullscreen me-2" /> Enter Fullscreen & Begin Interview
</button>
</div>
</div>
);
...
<div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
{[
{ icon: <i className="bi bi-file-text-fill" />, text: 'AI reads your resume and finds your tech skills' },
{ icon: <i className="bi bi-laptop" />, text: 'Technical questions — explain concepts and what you have built' },
{ icon: <i className="bi bi-people-fill" />, text: 'HR questions — time management, pressure, teamwork' },
{ icon: <i className="bi bi-shield-lock-fill" />, text: 'Anti-cheat: no tab switching, copy-paste, or external searches' },
{ icon: <i className="bi bi-trophy-fill" />, text: 'Get a score out of 10 with detailed feedback' },
].map((item, i) => (
<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', background: '#f9fafb', borderRadius: 10, fontSize: 14 }}>
<span style={{ fontSize: 20 }}>{item.icon}</span>
<span style={{ color: '#374151' }}>{item.text}</span>
...
<span><i className="bi bi-exclamation-triangle-fill me-1" /> {violations.length} integrity violation{violations.length > 1 ? 's' : ''} recorded</span>
<span style={{ color: '#fcd34d', fontWeight: 700 }}>
{MAX_WARNINGS - warningCountRef.current} warning{MAX_WARNINGS - warningCountRef.current !== 1 ? 's' : ''} remaining
</span>
</div>
...
<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
<span style={{ background: 'rgba(255,255,255,0.25)', borderRadius: 20, padding: '3px 14px', fontSize: 12, fontWeight: 700 }}>
{isHRQuestion ? <><i className="bi bi-people-fill me-1" /> HR / Behavioral</> : <><i className="bi bi-code-slash me-1" /> Technical</>}
</span>
{currentQ.topic && (
<span style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 20, padding: '3px 12px', fontSize: 12 }}>
{currentQ.topic}
...
<p style={{ fontSize: 12, opacity: 0.75, margin: '14px 0 0' }}>
{isHRQuestion
? <><i className="bi bi-lightbulb me-1" /> Give a real example — explain the situation, what you did, and the result.</>
: <><i className="bi bi-lightbulb me-1" /> Explain in your own words and mention how you used this in real projects.</>}
</p>
</div>
) : (
<div style={{ textAlign: 'center', padding: 40 }}>
...
<p style={{ fontSize: 13, color: '#374151', margin: 0 }}>{lastEval.feedback}</p>
{lastEval.strengths?.length > 0 && <p style={{ fontSize: 12, color: '#059669', margin: '4px 0 0' }}><i className="bi bi-check-lg me-1" /> {lastEval.strengths.join(' · ')}</p>}
{lastEval.gaps?.length > 0 && <p style={{ fontSize: 12, color: '#dc2626', margin: '2px 0 0' }}><i className="bi bi-x-lg me-1" /> {lastEval.gaps.join(' · ')}</p>}
</div>
)}
<div style={{ border: '1px solid #e5e7eb', borderRadius: 16, padding: 24, background: '#fafafa' }}>
<div style={{ textAlign: 'center', marginBottom: 16 }}>
...
<p style={{ fontSize: 13, color: '#6b7280', marginTop: 10, marginBottom: 0 }}>
{listening ? <><i className="bi bi-record-circle-fill text-danger me-1" /> Listening... click to stop</> : 'Click mic to speak, or type your answer below'}
</p>
</div>
<textarea
className="form-control mb-3"
...
if (step === STEPS.RESULT && result) {
const verdictConfig = {
shortlisted: { gradient: 'linear-gradient(135deg,#059669,#047857)', icon: <i className="bi bi-trophy-fill" />, label: 'Shortlisted!' },
maybe: { gradient: 'linear-gradient(135deg,#d97706,#b45309)', icon: <i className="bi bi-bar-chart-steps" />, label: 'Borderline' },
rejected: { gradient: 'linear-gradient(135deg,#dc2626,#b91c1c)', icon: <i className="bi bi-x-circle-fill" />, label: 'Not Selected' },
};
const vc = verdictConfig[result.verdict] || verdictConfig.rejected;
return (
<div className="page-content">
...
{terminated && (
<div style={{ background: '#7f1d1d', borderRadius: 12, padding: '14px 20px', marginBottom: 20, color: '#fca5a5', fontSize: 14 }}>
<i className="bi bi-stop-circle-fill me-2" /> This interview was auto-submitted after {MAX_WARNINGS} integrity violations.
</div>
)}
<div style={{ background: vc.gradient, borderRadius: 20, padding: '36px 32px', textAlign: 'center', color: '#fff', marginBottom: 24 }}>
<div style={{ fontSize: 52, marginBottom: 8 }}>{vc.icon}</div>
<h3 style={{ fontWeight: 700, margin: '0 0 4px' }}>{vc.label}</h3>
<div style={{ fontSize: 48, fontWeight: 700, margin: '8px 0' }}>
{result.total_score}<span style={{ fontSize: 20, opacity: 0.8 }}>/10</span>
</div>
...
<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
<div style={{ background: '#ede9fe', borderRadius: 14, padding: '18px 20px', textAlign: 'center' }}>
<div style={{ fontSize: 11, color: '#6d28d9', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}><i className="bi bi-code-slash me-1" /> Technical</div>
<div style={{ fontSize: 34, fontWeight: 800, color: '#5b21b6' }}>{result.technical_score ?? '—'}<span style={{ fontSize: 16, opacity: 0.7 }}>/10</span></div>
<div style={{ fontSize: 12, color: '#7c3aed', marginTop: 2 }}>3 questions</div>
</div>
<div style={{ background: '#fef3c7', borderRadius: 14, padding: '18px 20px', textAlign: 'center' }}>
<div style={{ fontSize: 11, color: '#92400e', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}><i className="bi bi-people-fill me-1" /> HR / Behavioral</div>
<div style={{ fontSize: 34, fontWeight: 800, color: '#b45309' }}>{result.hr_score ?? '—'}<span style={{ fontSize: 16, opacity: 0.7 }}>/10</span></div>
<div style={{ fontSize: 12, color: '#d97706', marginTop: 2 }}>2 questions</div>
</div>
</div>
...
{a.type === 'hr' ? <><i className="bi bi-people-fill me-1" /> HR</> : <><i className="bi bi-code-slash me-1" /> Technical</>}
</span>
<span style={{ fontSize: 13, fontWeight: 600 }}>
Q{i + 1}: {a.question?.length > 65 ? a.question.slice(0, 65) + '...' : a.question}
</span>
...
<p style={{ fontSize: 13, color: '#374151', margin: 0 }}>{a.feedback}</p>
{a.strengths?.length > 0 && <p style={{ fontSize: 12, color: '#059669', margin: '6px 0 0' }}><i className="bi bi-check-lg me-1" /> {a.strengths.join(' · ')}</p>}
{a.gaps?.length > 0 && <p style={{ fontSize: 12, color: '#dc2626', margin: '4px 0 0' }}><i className="bi bi-x-lg me-1" /> {a.gaps.join(' · ')}</p>}
</div>
))}
</>
)}
